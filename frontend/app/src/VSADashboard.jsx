...
export default function VSADashboard() {
  ...
  const [apiKeys, setApiKeys] = useState([
    { name: "Production App", key: "aegis_live_••••••••8x9q", date: "Oct 12, 2026", status: "Active" },
    { name: "Local Testing", key: "aegis_test_••••••••p2m4", date: "Sep 19, 2026", status: "Active" }
  ]);
      ...
      for(let i = 0; i < 90; i++) {
        ...
        transactions.forEach(t => {
           if(t.due_date === dateStr) {
              txDelta += (t.type === 'income' ? t.amount : -t.amount);
           }
        });
        
        currentBal = currentBal + txDelta - variableExpense;
        
        mockTrajectory.push({
          date: dateStr,
          balance: currentBal,
          bestCase: currentBal + (i * 20),
          worstCase: currentBal - (i * 30)
        });
      }
        ...
        {activeModel === 'b2b' && (
          <div className="bg-[#121214]/80 backdrop-blur-md p-6 rounded-2xl border border-amber-500/30 mb-8 shadow-[0_0_40px_rgba(245,158,11,0.15)] relative overflow-hidden">
            ...
            <div className="flex justify-between items-center mb-6 relative z-10">
              ...
              <button onClick={handleGenerateKey} className="bg-amber-500/10 text-amber-400 border border-amber-500/50 hover:bg-amber-500/20 px-4 py-2 rounded-lg font-semibold transition-all text-sm flex items-center space-x-2">
                ...
                <tbody className="divide-y divide-white/5">
                  {apiKeys.map((k, idx) => (
                    <tr key={idx} className="hover:bg-white/5 transition-colors">
                      ...
                      <td className="px-4 py-3 font-mono text-amber-400/80">{k.key}</td>
                      <td className="px-4 py-3">{k.date}</td>
                      <td className="px-4 py-3"><span className="bg-emerald-500/10 text-emerald-400 px-2 py-1 rounded-md text-xs font-semibold">{k.status}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
